...
在 **Windows** 上从**另一个项目目录**中随机选取一个源码文件，抽取**连续 10 行**，写入本仓库（脚本所在目录）下的 `snippets\` 中。不执行 Git 提交或推送。
...
3. 运行结束后窗口会 **暂停**，按任意键关闭；成功时会看到类似：`Done. Wrote: ...\snippets\snippet_....txt`。
...
- **文件名**：`snippet_yyyyMMdd_HHmmss_<6位随机十六进制>.txt`
- **内容**：
  - 前两行注释：`# source: <源文件完整路径>`、`# lines: <起始行>-<结束行>`
  - 空行
  - 连续 10 行正文（UTF-8，无 BOM）
...
| `random_copy_push.ps1` | 随机选文件、写 `snippets` 下的新文件 |
